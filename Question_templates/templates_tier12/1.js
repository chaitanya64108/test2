...
(function(){
  ...
  H.base('sq_mixed_num', function(){
    ...
    return {question:tri('Simplify '+Q+'.',Q+' ko simplify karo.',Q+' को सरल कीजिए।'),ans:K(FR(ps,qs)),w:[K(FR(ps+1,qs)),K(FR(ps-1>0?ps-1:ps+2,qs)),K(FR(ps,qs+1))],solution:buildMixedSimplifySolution(ps,qs)};
  });
  H.vars('sq_mixed_num',[
    function(){ var p=rint(2,12),q=rint(2,8); var g=gcd(p,q); var ps=p/g,qs=q/g; if(qs===1)qs=2; var num=ps*ps,den=qs*qs; return {question:tri('Find the value of '+frRootQ(num,den)+'.',frRootQ(num,den)+' ka value nikaalo.',frRootQ(num,den)+' का मान ज्ञात कीजिए।'),ans:K(FR(ps,qs)),w:[K(FR(ps+1,qs)),K(FR(ps-1>0?ps-1:ps+2,qs)),K(FR(ps,qs+1))],solution:buildMixedSimplifySolution(ps,qs)}; },
    function(){ var p=rint(2,10),q=rint(2,6); var g=gcd(p,q); var ps=p/g,qs=q/g; if(qs===1)qs=2; var num=ps*ps,den=qs*qs; var ans=(ps/qs).toFixed(2); return {question:tri('Express '+frRootQ(num,den)+' as a decimal.',frRootQ(num,den)+' ko decimal mein likho.',frRootQ(num,den)+' को दशमलव में लिखिए।'),ans:ans,w:[(ps/qs+0.5).toFixed(2),(ps/qs>0.5?ps/qs-0.5:ps/qs+1).toFixed(2),(ps/qs+1).toFixed(2)],solution:buildMixedDecimalSolution(ps,qs,ans)}; },
    function(){ var ps=rint(2,8),qs=2; var num=ps*ps,den=qs*qs; return {question:tri('For which value is '+K(RT('x'))+' = '+K(FR(ps,qs))+'?','Kis value ke liye '+K(RT('x'))+' = '+K(FR(ps,qs))+' hai?','किस मान के लिए '+K(RT('x'))+' = '+K(FR(ps,qs))+' है?'),ans:K(FR(num,den)),w:[K(FR((ps+1)*(ps+1),den)),K(FR(num,den+1)),K(FR(num+1,den))],solution:buildMixedFindXSolution(ps,qs,num,den)}; },
    function(){ var a=rint(2,8),b=rint(2,8); while(b===a)b=rint(2,8); return {question:tri(K(RT(FR(a*a,4)))+' + '+K(RT(FR(b*b,4)))+' = ?',K(RT(FR(a*a,4)))+' + '+K(RT(FR(b*b,4)))+' = ?',K(RT(FR(a*a,4)))+' + '+K(RT(FR(b*b,4)))+' = ?'),ans:K(FR(a+b,2)),w:[K(FR(a+b+1,2)),K(FR(a+b,3)),K(FR(Math.abs(a-b),2))],solution:buildMixedSumSolution(a,b)}; },
    function(){ var p1=rint(2,8),p2=rint(2,8); while(p2===p1)p2=rint(2,8); var v1=p1/2,v2=p2/2; var A=K(RT(FR(p1*p1,4))),B=K(RT(FR(p2*p2,4))); return {question:tri('Which is greater: '+A+' or '+B+'?','Bada kaun: '+A+' ya '+B+'?','बड़ा कौन: '+A+' या '+B+'?'),ans:v1>v2?A:B,w:['Equal',v1>v2?B:A,'Cannot determine'],solution:buildMixedCompareSolution(p1,p2)}; }
  ]);
})();
