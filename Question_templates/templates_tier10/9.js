...
(function(){
  ...
  H.vars('cb_decimal',[
    ...
    function(){ var a=rint(1,9); var n=a*a*a*1000000; var ans=a*100; var w=[ans+100,(ans-100>0?ans-100:ans+200),ans+50]; var sol=buildCbDecimalV3Solution(a,n,ans); return {question:tri('Find the value of '+K(CRT(n/1000000000))+'.',K(CRT(n/1000000000))+' ki value nikalo.',K(CRT(n/1000000000))+' का मान ज्ञात कीजिए।'),ans:String(ans),w:w.map(String),solution:sol}; },
  ...
  H.base('cb_make_x', function(){
    ...
    else if(variant===1){ var pr=[2,3,5,7,11,13,17]; var pp=shuffle(pr).slice(0,2); var p1=pp[0],p2=pp[1]; num=p1*p2*p2; ans=p1*p1; w1=p1; w2=p2*p2; w3=p1+p2; }
    else { var pr2=[2,3,5,7,11,13]; var pp2=shuffle(pr2).slice(0,2); var q1=pp2[0],q2=pp2[1]; var q3=pick(pr2.filter(function(p){return p!==q1&&p!==q2;})); num=q1*q2*q3*q3; ans=q1*q2; w1=q1*q2*2; w2=q3; w3=q1+q2; }
    var sol=buildCbMakeXSolution(num,ans,variant);
    return {question:tri('By what number should '+K(num)+' be multiplied to make it a perfect cube?',K(num)+' ko kis number se multiply karein ki perfect cube bane?',K(num)+' को किस संख्या से गुणा करें कि पूर्ण घन बने?'),ans:String(ans),w:[String(w1),String(w2),String(w3)],solution:sol};
  });
  H.vars('cb_make_x',[
    function(){ var p=pick([2,3,5,7]); var k=rint(2,6); var num=p*k*k; var ans=num*p; var sol=buildCbMakeXSolution(num,ans,0); return {question:tri('Multiply '+K(num)+' by '+K(p)+'. Is the result a perfect cube?',K(num)+' ko '+K(p)+' se multiply karo. Kya result perfect cube hai?',K(num)+' को '+K(p)+' से गुणा करो। क्या परिणाम पूर्ण घन है?'),ans:'Yes — '+ans,w:['No','Yes — '+(ans+p),'Yes — '+(ans+k)],solution:sol}; },
    function(){ var a=rint(2,6),b=rint(2,6); var na=a*b*b, nb=a*a*b; var ans=a*b; var w=[a*b+1,(a*b-1>0?a*b-1:a*b+2),a+b]; var sol=buildCbMakeXSolution(na,ans,1); var expr=K(na+T+nb); return {question:tri(expr+' is the perfect cube of which number?',expr+' kis number ka perfect cube hai?',expr+' किस संख्या का पूर्ण घन है?'),ans:String(ans),w:w.map(String),solution:sol}; },
    function(){ var base=rint(2,6),sqPart=rint(2,6); var num=base*sqPart*sqPart; var ans=base; var w=[base*2,sqPart,base+sqPart]; var sol=buildCbMakeXSolution(num,ans,2); var expr=K(num+T+'?'); return {question:tri(expr+' is a perfect cube. Which value works for the blank?',expr+' perfect cube hai. Blank mein kaunsa value aayega?',expr+' पूर्ण घन है। रिक्त स्थान में कौन सा मान आएगा?'),ans:String(ans),w:w.map(String),solution:sol}; },
    function(){ var base=rint(2,8),sqPart=rint(2,5); var num=base*sqPart*sqPart; var ans=base; var w=[base*sqPart,sqPart*sqPart,base*base]; var sol=buildCbMakeXSolution(num,ans,3); return {question:tri('What is the smallest number you can multiply '+K(num)+' by to get a perfect cube?',K(num)+' ko perfect cube banane ke liye sabse chhota multiplier kya hai?',K(num)+' को पूर्ण घन बनाने के लिए सबसे छोटा गुणक क्या है?'),ans:String(ans),w:w.map(String),solution:sol}; },
    function(){ var p=pick([2,3,5]); var k=rint(2,5); var num=p*k*k; var ans=k; var w=[k+1,(k-1>0?k-1:k+2),p*k]; var sol=buildCbMakeXSolution(num,ans,4); return {question:tri('After multiplying '+K(num)+' by '+K(p)+', what is the cube root of the result?',K(num)+' ko '+K(p)+' se multiply karne ke baad result ka cube root kya hai?',K(num)+' को '+K(p)+' से गुणा करने के बाद परिणाम का घनमूल क्या है?'),ans:String(ans),w:w.map(String),solution:sol}; }
  ]);
})();
